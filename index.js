...
var Kiss = class {
  constructor() {
    __publicField2(this, "platform", "unknown");
    __publicField2(this, "ui_client", 3);
...
var MyEvent = class extends EventEmitter {
  ...
  answer(event2, handler) {
    return super.on(event2, handler);
  }
  query(event2, data) {
    this.send(event2, data);
  }
};
...
const event = __spreadValues(__spreadValues(__spreadValues({}, notifyEvent()), commonEvent()), customEvent());
function dataToString(data) {
  if (typeof data !== "string")
    data = JSON.stringify(data);
  return data;
}
io_hook == null ? void 0 : io_hook.on(event.NOTIFY, (data) => {
...
io_hook == null ? void 0 : io_hook.on(event.WARN, (data) => {
  data = dataToString(data);
  if (env.inMg) {
    client.mg.notify(data, {
      position: "bottom",
...
function allowMg(target, propertyKey, descriptor) {
...
function allowFg(target, propertyKey, descriptor) {
  const originalMethod = descriptor.value;
  descriptor.value = function(...args) {
    if (!env.inMg)
      originalMethod.apply(this, args);
...
__decorateClass$1([
  allowMg
], SelParser.prototype, "getMgSel", 1);
__decorateClass$1([
  allowFg
], SelParser.prototype, "getFgSel", 1);
function getConfig(cacheName2 = "cache_name", initConfig2) {
  return __async(this, null, function* () {
    var _a;
    return (_a = yield client.mg.clientStorage.getAsync(cacheName2)) != null ? _a : initConfig2;
  });
}
function updateConfig(cacheName2, newConfig) {
  return __async(this, null, function* () {
    yield client.mg.clientStorage.setAsync(cacheName2, newConfig);
  });
}
...
const cache = ref();
function firstParserConfig() {
  return __async(this, null, function* () {
    if (!cache.value) {
      cache.value = yield getConfig(cacheName, initConfig);
    }
    return cache.value;
  });
}
...
class IconResizer {
  ...
  checkNoSelection() {
    if (this.frameAndComponentNodes.length === 0) {
      io_hook == null ? void 0 : io_hook.emit(event.WARN, "\u{1F971}\u6CA1\u6709\u627E\u5230\u53EF\u53D8\u6362\u7684\u56FE\u6807");
      return true;
    }
    return false;
  }
  successTip(num = 1) {
    io_hook == null ? void 0 : io_hook.emit(event.NOTIFY, `\u{1F389} \u5DF2\u89C4\u8303 ${num} \u4EFD\u56FE\u6807\u7684\u5927\u5C0F`);
  }
  updateCacheConfig() {
  ...
  run() {
    ...
    this.frameAndComponentNodes.forEach((node) => {
...
class NodeResizer {
  ...
  changeGroupToFrame(node) {
    var _a, _b;
    if (node.type !== "GROUP")
      return node;
    const frame = figma.createFrame();
    (_b = node.parent) == null ? void 0 : _b.insertChild((_a = node.parent) == null ? void 0 : _a.children.indexOf(node), frame);
    frame.x = node.x;
    frame.y = node.y;
    frame.fills = [];
  ...
  groupChildren() {
    ...
    if (childrenLength > 1) {
      if (env.inMg)
        client.mg.group(this.cloneNode.children);
      else
        client.figma.group(this.cloneNode.children, this.cloneNode);
    }
  }
  get firstChild() {
...
__decorateClass([
  allowMg
], NodeResizer.prototype, "changeWH_mg", 1);
__decorateClass([
  allowFg
], NodeResizer.prototype, "changeWH_figma", 1);
__decorateClass([
  allowMg
], NodeResizer.prototype, "changeBox_mg", 1);
__decorateClass([
  allowFg
], NodeResizer.prototype, "changeBox_figma", 1);
if (env.inMg) {
...
io_hook == null ? void 0 : io_hook.on(event.UI_CLOSE, () => {
  client.mg.closePlugin();
});
io_hook == null ? void 0 : io_hook.on(event.UI_INIT, () => {
  firstParserConfig().then((config) => {
    console.log("config", config);
    io_hook == null ? void 0 : io_hook.send(event.CODE_INIT_CONFIG, config);
  });
});
io_hook == null ? void 0 : io_hook.answer("UI_CHANGE_SIZE", (data) => {
